...
// The Astro config every `tools/compat/astro-*` fixture builds with. It is copied into each
// fixture's `src/` with the rest of this consumer and re-exported by that fixture's
// `astro.config.mjs`, so every Astro major builds the same routing, the same catalog server
// and the same Vite plugins - only the fixture's `package.json` differs. Bare imports resolve
// from the fixture's own `node_modules`, and paths below are relative to `<fixture>/src/`.
...
const catalogs = createServer((request, response) => {
  ...
  try {
    const body = readFileSync(new URL(`./i18n/${locale}.json`, import.meta.url), 'utf8');
...
export default defineConfig({
  ...
  vite: {
    ...
    plugins: [
      ...
      // `@etyma/tooling/vite` declared where an Astro site declares any Vite plugin: the
